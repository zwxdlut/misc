...
def process_data():
    ...
    val_csv_path=root_path + "/new_val.csv"
...
def calculate_split_info(path: str, label_dict: dict, rate: float = 0.2):
    ...
    with open(root_path + '/classes_name.json', 'w') as json_file:
...
def main():
    json_path = root_path + "/imagenet_class_index.json"  # 指向imagenet的索引标签文件

    # load imagenet labels
    label_dict = json.load(open(json_path, "r"))
    label_dict = dict([(v[0], v[1]) for k, v in label_dict.items()])

    calculate_split_info(root_path, label_dict)
